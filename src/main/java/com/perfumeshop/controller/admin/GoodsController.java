...
import com.perfumeshop.util.ImageUtil;
...
public class GoodsController {
    ...
    public String addGoods(Goods goods,
        ...
        for (MultipartFile multipartFile : fileToUpload) {
            ...
            System.out.println("最后存入数据的图片名字为:"+fileName);
            if (multipartFile != null) {
                //获取文件存入路径
                String str = request.getSession().getServletContext().getRealPath("/");
                String filePath= str.substring(0,str.indexOf("target"))+"/src/main/webapp/pictures";
               String ImagePath= ImageUtil.imagePath(multipartFile,fileName,filePath);
